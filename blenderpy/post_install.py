...
def install_scripts_directory():
    ...
    if blender_scripts_current_dir is not None:
        ...
        if (
            str(pathlib.Path(blender_scripts_current_dir).parent.absolute()).casefold()
            == blender_scripts_install_dir.casefold()
        ):

            print(
                blender_scripts_current_dir
                + " already direct child of "
                + blender_scripts_install_dir
            )
