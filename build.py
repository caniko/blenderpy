...
class InstallBlenderScripts(install_scripts):
    ...
    def run(self):
        ...
        for scripts_dir in scripts_dirs:
            ...
            shutil.copytree(
                scripts_dir, os.path.join(self.build_dir, os.path.basename(scripts_dir))
            )
...
class BuildCMakeExt(build_ext):
    ...
    def finalize_options(self):
        ...
        self.set_undefined_options(
            ...
            ("bpy_cmake_configure_args", "bpy_cmake_configure_args"),
        )
    ...
    def run(self):
        ...
        for extension in self.extensions:
            ...
            if isinstance(extension, CMakeExtension):
                ...
                self.announce(
                    f"Preparing the build environment for CMake "
            ...
            if extension.name == "bpy":
                ...
                if self.bpy_prebuilt:  # user assumes responsibility
                    ...
                    self.announce(
                        f"Using supplied prebuilt path " f"{self.bpy_prebuilt}", level=3
                    )
                    ...
                    git_checkout_path = pathlib.Path(
                        os.path.join(self.build_temp, "blender")
                    )
    ...
    def build_bpy(
        self,
        git_checkout_path: pathlib.Path,
        svn_checkout_path: pathlib.Path,
        ...
        if not VERSION_TUPLE in compatible_bpy:
            ...
            raise Exception(
                f"{VERSION} bpy is not compatible with "
        ...
        self.announce(
            "Cloning Blender source from git " "(this will take a while)", level=3
        )

        git_repo.checkout(git_checkout_path)  # Clones into "blender"

        self.announce(
            "Cloning precompiled libs from svn " "(this will take a while)", level=3
        )

        svn_repo.checkout(svn_checkout_path)  # Checkout into "lib" (automatic)

        self.announce(
            "Configuring cmake project and building binaries "
            "(this will take a while)",
            level=3,
        )

        for command in bpybuild.make.get_make_commands(
            source_location=git_checkout_path,
    ...
    def copy_bpy(self, source_path: pathlib.Path, dest_path: pathlib.Path):
        ...
        bpy_canidates = [
            os.path.join(source_path, _bpy)
            for _bpy in os.listdir(source_path)
            if os.path.isfile(os.path.join(source_path, _bpy))
            and os.path.splitext(_bpy)[0].startswith("bpy")
            and os.path.splitext(_bpy)[1] in [".pyd", ".so"]
        ]
...
def build(setup_kwargs):
    ...
    subprocess.check_call([sys.executable, "-m", "pip", "install", "bpy-build", "wheel"])
